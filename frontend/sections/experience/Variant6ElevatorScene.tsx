...
import { PART_ASSETS, SPINE_ASSET } from "./variants/partAssets";
import { EXPLORATION_PARTS } from "@/data/heroExploration";
import story from "./ScrollStory.module.css";
import styles from "./ElevatorScene.module.css";

/* VARIANT 6 — an exact duplicate of the homepage hero (ElevatorScene): same
   night-city arrival, dolly-zoom, threshold, camera choreography, day/night
   cycle and outro. The ONLY change is inside the cabin — the modelled interior
   detail and the 8 procedural component sub-parts are replaced with the real
   photoreal renders (the cutaway machine "spine" + the 8 component cutouts),
   hung at the exact FRAMING anchors the camera already flies to and frames.
   Everything outside the two clearly-marked "V6" blocks is byte-identical to
   ElevatorScene.tsx, so the arrival experience is unchanged. */
...
export function Variant6ElevatorScene() {
    ...
    <section ref={root} className={styles.section} style={{ height: `${SCROLL_VH}vh` }} aria-label="Arrival at a Philbrick-equipped tower and interactive elevator, explore the components">
      <div className={styles.stage}>
        ...
        <div ref={v6Overlay} className={styles.v6Overlay} style={{ opacity: 0 }}>
          ...
          <img ref={v6SpineRef} src={SPINE_ASSET.src} alt="" className={styles.v6Spine} style={{ opacity: 0 }} />
          {EXPLORATION_PARTS.map((part, i) => {
            const asset = PART_ASSETS.find((a) => a.key === part.key);
            return (
              <button
                key={part.key}
                ref={(el) => {
                ...
                style={{ left: `${part.slot.x}%`, top: `${part.slot.y}%`, opacity: 0 }}
                onClick={() => setOpenComponent(part.component)}
                aria-label={`${part.component.name}, view details`}
              >
                <img src={asset?.src ?? part.image} alt="" className={styles.v6PartImg} />
