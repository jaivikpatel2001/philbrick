...
/* SpineConfig is a union; the catalogue spine is always the image variant */
const CATALOG_SPINE_SRC = CATALOG_SPINE.type === "image" ? CATALOG_SPINE.src : "";
import story from "./ScrollStory.module.css";
import styles from "./ElevatorScene.module.css";

/* VARIANT 12 — a duplicate of variant6 (which itself preserves the homepage
   hero's night-city arrival, dolly-zoom, threshold, camera choreography,
   day/night cycle and outro, 100% of the GSAP timeline). The ONLY change from
   variant6: the exploded DOM overlay now shows the client's CATALOGUE assets —
   the technical drawing of the whole installation as the centre spine and the
   9 catalogue part cutouts (data/catalogParts.ts), each flying to its
   documented position with a leader line back to where it lives on the
   drawing. Parts are informational (no modal — the catalogue set has no
   spec-sheet content, and empty modals would read broken). */
...
/* V6: zigzag slots that frame the elevator image (variant1-style), where the 8
   component cutouts land one by one. World coords around the spine at origin;
   also used as the hotspot anchors so the "details" chips sit on each part. */
const V6_SLOTS: [number, number, number][] = [
  [-1.75, 0.85, 0.45],
  [1.75, 0.85, 0.45],
  [-2.05, 0.05, 0.45],
  [2.05, 0.05, 0.45],
...
function activeFromP(p: number) {
  ...
  const step = (COMP_END - COMP_START) / CATALOG_PARTS.length;
  return Math.min(CATALOG_PARTS.length - 1, Math.max(0, Math.floor((p - COMP_START) / step)));
}
function hasWebGL() {
  try {
...
export function Variant12ElevatorScene() {
...
function Scene3D({ onContextFail }: { onContextFail: () => void }) {
  ...
  // V6 exploded-view overlay (variant1 layout) refs
  const v6Overlay = useRef<HTMLDivElement>(null);
  const v6SpineRef = useRef<HTMLImageElement>(null);
  const v6PartRefs = useRef<(HTMLButtonElement | null)[]>([]);
  const v6LineRefs = useRef<(SVGLineElement | null)[]>([]);
  ...
  useEffect(() => {
    ...
    /* ===== V6 BLOCK 1: the modelled elevator is removed entirely ===========
       The whole 3D cabin (car group: shell, doors, header, COP, brand decal,
       lights, brakes) is hidden, and the shaft (rails/counterweight/machine) is
       hidden where it is built. The 3D scene now only plays the building
       arrival; once inside, a DOM overlay (see the JSX + the v6 reveal ticker)
       presents the real cutaway render + the 8 component cutouts in variant1's
       exact exploded layout (screen-% slots, leader lines, labels). Doing the
       exploded view in the DOM — not billboarded 3D planes — is what keeps the
       parts perfectly placed and non-overlapping, exactly like /variant1. */
    car.visible = false;
    /* ===== end V6 BLOCK 1 ================================================== */
    ...
    /* V6: hide the whole 3D shaft (rails, counterweight, machine, ropes) too —
       the real cutaway render is the elevator now. */
    shaft.visible = false;
    ...
    const stops = [
      ...
      // V6: settle inside the lobby FACING the elevator image and hold there,
      // with a slow push-in while the 8 component images reveal one by one
      // around it — no flying around a cabin that no longer exists.
      { p: INSIDE_P, pos: [0, 0.3, 9.2], target: [0, 0.1, 0] },
      { p: 0.4, pos: [0, 0.2, 7.4], target: [0, 0.05, 0] },
      { p: COMP_START, pos: [0, 0.15, 6.6], target: [0, 0.05, 0] },
      { p: COMP_END, pos: [0, 0.15, 5.6], target: [0, 0.05, 0] },
      { p: 1.0, pos: [0, 0.1, 5.2], target: [0, 0.05, 0] },
    ] as { p: number; pos: number[]; target: number[] }[];
    void frameOf;
    void step;
    // V6: hotspot chips project onto the zigzag slots (where the parts land)
    const anchors = V6_SLOTS.map((s) => new THREE.Vector3(...s));
    ...
    const pose = (p: number) => {
      ...
      /* V6: the exploded component view is a DOM overlay (see the reveal ticker
         in Scene3D), so there is no 3D part reveal to run here. */
      updateHotspots();
    };
  ...
  /* V6: drive the DOM exploded-view overlay off scroll progress. The 3D scene
     plays the building arrival; from the threshold the overlay fades in, the
     cutaway render centres, then the 8 component cutouts reveal one by one —
     each flying from its anchor on the spine to its resting slot with a leader
     line — and ACCUMULATE, exactly like /variant1. Screen-% layout (not 3D)
     is what keeps them perfectly placed and non-overlapping. */
  useEffect(() => {
    const parts = CATALOG_PARTS;
    const step = (COMP_END - COMP_START) / parts.length;
    const tick = () => {
      const p = progress.current;
      const enter = smoothstep(0.3, 0.42, p);
      if (v6Overlay.current) v6Overlay.current.style.opacity = String(enter);
      if (v6SpineRef.current) {
        const s = smoothstep(0.32, 0.44, p);
        v6SpineRef.current.style.opacity = String(s);
        v6SpineRef.current.style.transform = `translate(-50%, -50%) scale(${lerp(0.92, 1, s)})`;
      }
      for (let i = 0; i < parts.length; i++) {
        const part = parts[i];
        const rs = COMP_START + i * step;
        const k = smoothstep(rs, rs + step * 0.62, p); // reveal, then hold (accumulate)
        const x = lerp(part.anchor.x, part.slot.x, k);
        const y = lerp(part.anchor.y, part.slot.y, k);
        const el = v6PartRefs.current[i];
        if (el) {
          el.style.left = `${x}%`;
          el.style.top = `${y}%`;
          el.style.opacity = String(k);
          el.style.pointerEvents = k > 0.6 ? "auto" : "none";
        }
        const line = v6LineRefs.current[i];
        if (line) {
          line.setAttribute("x2", String(x));
          line.setAttribute("y2", String(y));
          line.style.opacity = String(smoothstep(rs + step * 0.35, rs + step * 0.72, p) * 0.5);
        }
      }
    };
  ...
  return (
    <section ref={root} className={styles.section} style={{ height: `${SCROLL_VH}vh` }} aria-label="Arrival at a Philbrick-equipped tower and interactive elevator, explore the components">
      <div className={styles.stage}>
        ...
        {/* V6 exploded component overlay — variant1's exact layout (screen-%
            slots, leader lines, labels), driven by the reveal ticker above.
            Fades in once the camera is inside the building. */}
        <div ref={v6Overlay} className={styles.v6Overlay} style={{ opacity: 0 }}>
          <div className={styles.v6Scrim} aria-hidden />
          <svg className={styles.v6Lines} viewBox="0 0 100 100" preserveAspectRatio="none" aria-hidden>
            {CATALOG_PARTS.map((part, i) => (
              <line
                key={part.key}
                ref={(el) => {
                  v6LineRefs.current[i] = el;
                }}
                x1={part.anchor.x}
                y1={part.anchor.y}
                x2={part.slot.x}
                y2={part.slot.y}
                className={styles.v6Line}
                style={{ opacity: 0 }}
              />
            ))}
          </svg>
          <img ref={v6SpineRef} src={CATALOG_SPINE_SRC} alt="" className={styles.v6Spine} style={{ opacity: 0 }} />
          {CATALOG_PARTS.map((part, i) => (
            <button
              key={part.key}
              ref={(el) => {
                v6PartRefs.current[i] = el;
              }}
              type="button"
              className={`${styles.v6Part} ${part.side === "left" ? styles.v6Left : styles.v6Right}`}
              style={{ left: `${part.slot.x}%`, top: `${part.slot.y}%`, opacity: 0, cursor: "default" }}
              aria-label={part.component.name}
            >
              <img src={part.image} alt="" className={styles.v6PartImg} />
              <span className={styles.v6Label}>
                <span className={styles.v6Idx}>{part.component.index}</span>
                <span className={styles.v6Name}>{part.component.name}</span>
                <span className={styles.v6Tag}>{part.component.tagline}</span>
              </span>
            </button>
          ))}
        </div>
        ...
        {/* V6: the 3D hotspots and the bottom-left caption are replaced by the
            exploded overlay's per-part labels above (variant1 layout). */}

        {/* Component rail (catalogue part index — informational, no modal) */}
