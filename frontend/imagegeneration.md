...
**Status: SUPPLIED & ART-DIRECTED (updated 2026-07-25).** The hero now uses a
LANDSCAPE + PORTRAIT pair per theme (client-supplied), not a single plate:

| File | Dimensions | Used on |
|---|---|---|
| `hero-scene-day-landscape.png` | 1672×941 | wide screens (light) |
| `hero-scene-day-portrait.png` | 864×1821 | narrow screens (light) |
| `hero-scene-night-landscape.png` | 1672×941 | wide screens (dark) |
| `hero-scene-night-portrait.png` | 852×1846 | narrow screens (dark) |

All in `public/images/home/hero-exploration/environment/`. `Variant18Hero.tsx`
renders them through a plain `<picture>` with a `(orientation: portrait)`
`<source>` — landscape is the default, portrait swaps in on narrow/portrait
viewports. WebP ladders are generated by **`scripts/optimizeHeroScene.mjs`**
(landscape 640–1672, portrait 384–820), NOT by `optimizeHeroExploration.mjs`
(which now skips these files). The old single 2752×1536 `hero-scene-{day,night}.png`
plates and their variants were deleted; the mobile `object-fit: contain`
letterbox hack was removed (the portrait plate is the fix it was waiting for).

The current baked headline is **"Your One Stop Elevator Solutions Partner."**
(navy "Your One Stop Elevator" + blue "Solutions Partner.") — it must match the
`sr-only <h1>` in `Variant18Hero.tsx`.

Original single-plate prompts are kept below for reference.
...
Line 1, clean white: Your One Stop Elevator
Line 2, bright azure blue #109BDD: Solutions Partner.
The text must sit behind the buildings, never on top of them. Spell the words exactly as written.
